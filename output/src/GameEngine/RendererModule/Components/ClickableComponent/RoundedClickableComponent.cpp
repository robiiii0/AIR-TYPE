...
Engine::RendererModule::Components::RoundedClickableComponent::
    ...
    _data(value), _isHovered(false), _isClicked(false), _func(func) {
    sf::RectangleShape shape(
        sf::Vector2f(value._size.first, value._size.second));
    _buttonShape = shape;
...
sf::Drawable &Engine::RendererModule::Components::RoundedClickableComponent::
    getDrawable() {
    return _buttonShape;
}
