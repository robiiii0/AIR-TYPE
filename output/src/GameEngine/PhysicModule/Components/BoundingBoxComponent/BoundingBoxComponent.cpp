...
Engine::Physic::Components::BoundingBoxComponent::BoundingBoxComponent(
    std::any value) {
    if (value.type() ==
        typeid(std::pair<std::pair<float, float>, std::pair<int, int>>)) {
        _component_name = "BoundingBoxComponent";
        _value = std::any_cast<
            std::pair<std::pair<float, float>, std::pair<int, int>>>(value);
    }
}
...
void Engine::Physic::Components::BoundingBoxComponent::setBoundingBox(
    std::pair<std::pair<float, float>, std::pair<int, int>> pos) {
    _pos = pos.first;
    _size = pos.second;
