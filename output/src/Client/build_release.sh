...
if [ ! -e "$CURRENT_DIR/release/Debug/r-type_client" ]; then
  ...
  built=built+1
  cp "$CURRENT_DIR/release/Debug/r-type_client" "$CURRENT_DIR/../../release/r-type_client"
fi
...
if [ ! -e "$CURRENT_DIR/release/Debug/r-type_client.exe" ]; then
  ...
  built=built+1
  cp "$CURRENT_DIR/release/Debug/r-type_client.exe" "$CURRENT_DIR/../../release/r-type_client.exe"
fi
