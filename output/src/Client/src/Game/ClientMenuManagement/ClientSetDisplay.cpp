...
void Client::setMenu() {
    ...
    createButton(std::bind(&Client::changeState, this, GameState::SETTINGS), "Setting",
                 _texturesButton[0], _fonts[0],
...
void Client::setSettings() {
    ...
    createParallax(_texturesParallax);
    createButton(
        std::bind(&Client::changeState, this, GameState::KEYBINDING), "Change Key", _texturesButton[0], _fonts[0],
        sf::Vector2f(_screenWidth / 2 - _texturesButton[0].getSize().x,
                     _screenHeight / 2 - _texturesButton[0].getSize().y / nb),
        sf::Vector2f(0.5, 0.5), sf::Color::White, 0);
    nb++;
    createButton(
        std::bind(&Client::changeState, this, GameState::MENU), "Menu", _texturesButton[0], _fonts[0],
        sf::Vector2f(_screenWidth / 2 - _texturesButton[0].getSize().x,
                     _screenHeight / 2 - _texturesButton[0].getSize().y / nb),
        sf::Vector2f(0.5, 0.5), sf::Color::White, 0);
    nb++;
